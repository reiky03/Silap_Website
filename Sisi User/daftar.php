...
if (isset($_POST['submit'])) { //untuk create
  ...
  if ($tnik && $gender && $umur && $nama && $email && $tgl && $tujuan && $ajg) {
    if (strlen($tnik) == 16 && is_numeric($tnik) && is_numeric($umur) && strlen($umur) <= 2) {
      //untuk insert data
      $masukkan = "INSERT INTO `pasien`(`id`, `nama`, `jenis_Kelamin`, `tnik`, `umur`, `email`, 
      `tgl_daftar`, `puskesmas`, `id_layanan`) 
      VALUES (NULL, '$nama', '$gender', '$tnik', '$umur', '$email','$tgl', '$tujuan','$ajg')";
      ...
      <div class="col-md-12 align-items-center mb-4 pt-3">
        ...
        <form class="p-4 shadow border rounded-4 text-light" method="post" action="">
          ...
          <div class="mb-3">
            ...
            <input type="text" autocomplete="off" class="form-control" id="exampleInputName" name="nama" value="<?php echo $nama ?>" title="Nama harus diisi" required>
          </div>
          <div class="mb-3">
            <label for="exampleInputPuskes" class="form-label">Jenis Kelamin</label>
            <select class="form-select form-select" name="gender" id="gender" title="Harap isi bidang ini" required>
            ...
            <div class="mb-3">
              ...
              <input type="text" autocomplete="off" class="form-control" id="exampleInputNIK" name="tnik" value="<?php echo $tnik ?>" maxlength="17" pattern="\d{16,16}" title="Berisi 16 digit number" required>
          </div>
          <div class="mb-3">
            <label for="exampleInputNIK" class="form-label">Umur</label>
            <input type="text" autocomplete="off" class="form-control" id="exampleInputUmur" name="umur" value="<?php echo $umur ?>" maxlength="3" pattern="^[1-9][0-9]?$" title="Masukkan umur yang valid antara 1 sampai 99" required>
          </div>
          <div class="mb-3">
            <label for="exampleInputEmail1" class="form-label">Email address</label>
            <input type="email" autocomplete="off" class="form-control" id="exampleInputEmail1" name="email" value="<?php echo $email ?>" required>
          </div>
          <div class="mb-3">
            <label for="exampleInputtgl1" class="form-label">Tanggal Daftar</label>
            <input type="date" autocomplete="off" class="form-control" id="exampleInputtgl1" name="tanggal" value="<?php echo $tgl ?>" required>
          </div>       
          <div class="mb-3">
